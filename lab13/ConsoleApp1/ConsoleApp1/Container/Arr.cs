...
namespace ConsoleApp1.Container
{
        ...
        where T : IComparable
    {
        ...
        public event EventHandler CountTotal;
        ...
        public void Add(T P)
        {
            ...
            //Такое приведение типа небезопасное, программа сломается если использовать другой класс, но другого решения я не нашел :(
            Product<string> forTotal = (Product<string>)Convert.ChangeType(P, typeof(Product<string>));
            Total += forTotal.Price;
        ...
        public void Delete(int index)
        {
            ...
            while (i <= Count)
            {
                ...
                {
                    //Такое приведение типа небезопасное, программа сломается если использовать другой класс, но другого решения я не нашел :(
                    Product<string> forTotal = (Product<string>)Convert.ChangeType(Container[i], typeof(Product<string>));
                    Total -= forTotal.Price;
                }
                if (i != index)
                {
        ...
        public T this[int i]
        {
            set {
                //Такое приведение типа небезопасное, программа сломается если использовать другой класс, но другого решения я не нашел :(
                Product<string> temp1 = (Product<string>)Convert.ChangeType(Container[i], typeof(Product<string>));
                Product<string> temp2 = (Product<string>)Convert.ChangeType(value, typeof(Product<string>));
                Total = Total - temp1.Price + temp2.Price;
                Container[i] = value;
                CountTotal?.Invoke(this, null);
            }
            get
